...
#class WeatherReport
  ...
  city = current_observation["display_location"]["city"] #response["current_observation"]["display_location"]["city"]
  state = current_observation["display_location"]["state"] #response["current_observation"]["display_location"]["state_name"]
  obs_time = current_observation["observation_time"]

  temperature = current_observation["temperature_string"]
  weather = current_observation["weather"]
  rain_last_hr = current_observation["precip_1hr_string"]
  rain_today = current_observation["precip_today_string"]
  narrative = "Weather report for #{city}, #{state} (#{obs_time}):\nConditions are #{weather.downcase}, with a current temperature of #{temperature}. There has been #{rain_last_hr} of precipation in the last hour for a total of #{rain_today} for the day."
  puts narrative
